%This is a code for Figures 2(a,b), 3(a,b).
% Figure 2(a):
% substitute  5*10^7 in the second position of Initial,
%substiture m=0; in line 42
% Figure 2(b):
%substiture m=28740; in line 42
% Figure 3(a):
% substitute  7.9*10^7 in the second position of Initial,
% Figure 3(b):
%substiture m=28740; in line 72
...
p4=1.44*10^(-5);
d0=1.032*10^3;%Adaptation from 10^5 to 10^4 to fullfill/satisfy the order
%of magnitude
...
Initial=[ 0 5*10^7 d0/mu2 (gamma*d0/mu2)/(mu3+(p4*eta*d0)/(mu2*mu4)) eta*d0/(mu2*mu4)]; %Simulations for proposal: [ 0 3.7*10^7 1*10^7 ];
y0 = Initial;
...
yticks([1e0 1e2 1e4 1e6 1e8 ])               % specify positions
yticklabels({'10^0','10^2','10^4','10^6','10^8'})  % specify labels
%ax.YAxis.Exponent = 2;
xlim([0 80])
ylim([0.05 1*10^8])
